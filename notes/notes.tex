...
If $j = 0$, then all the summands are equal to one, so get just $(2k+1)/N$.
...
\end{equation}
\[\eqref{eq:lpf before trig id} =\frac{\sin((k+1/2)x_j/2)}{\sin(x_j/2)}\]
\[y_j = \begin{cases}
	\frac{1}{\sqrt{N}}\frac{\sin((k+1/2)x_j/2)}{\sin(x_j/2)} \; \forall j\in {1, \dots, N-1} \\
	(2k+1)/N, \; j = 0
\end{cases}\]
