...
\begin{proof}
    \[\begin{aligned}
            {(F \overline{F^T})}_{jl}
            = \frac{1}{\sqrt N}\overline{\left(\frac{1}{\sqrt N}\right)}\sum_{k=0}^{N-1}F_{lk}{(\overline{F^T})}_{kl}
    ...
    \begin{enumerate}
        \item $j \neq l$. Using the formula for sum of geometric series:
              \[
                  \sum_{k=0}^{N-1}{\left( \omega_N^{(j-l)k} \right)}^{k}
                  = \frac{1 - {\left(\omega_N^{(j-l)k}\right)}^N}{1 - \omega^{(j-l)k}}
                  = \frac{1 - {\left(\omega_N^{N}\right)}^{(j-l)k}}{1 - \omega^{(j-l)k}}
                  \underset{(a)}{=} \frac{1 - 1^{(j-l)k}}{1 - \omega^{(j-l)k}}
                  = 0\]
    ...
    The matrix $F\overline{F} = I$, so $F$ unitary and $F^{-1}=\overline{F^T}$

    The entries of $F^{-1}$ arre given by
    $F^{-1}_{ij} = (\overline{F_{ji}}) = \overline{\omega_N^{ji}} = \omega_N^{-ij}$.
\end{proof}
...
\begin{theorem}[Real signal conjugate symmetry] \label{thm: real signal symmetry}
    ...
    \begin{enumerate}
        ...
        \item $z_{j} = \overline{z_{N-j}} \;\forall j \in \{1, \dots, N/2-1\}$ and $z_0, z_{N/2} \in \R$
    \end{enumerate}
\end{theorem}
\begin{proof} \hfill \break
    ...
    \[\begin{aligned}
            ...
            \underset{\omega_N^N=1}{=} \overline{ \left(\frac{1}{\sqrt N} \sum_{k=0}^{N-1} \omega_N^{-N}\omega_N^{kj} y_k \right)}
             & = \\ \overline{ \left(\frac{1}{\sqrt N} \sum_{k=0}^{N-1} \omega_N^{-(N-kj)} y_k \right)}
            = \overline{z_{N-j}}
        \end{aligned}\]
    \[z_0
    ...
    \begin{equation}
        \begin{aligned}
            y_j
            = \frac{1}{\sqrt{N}}\sum_{k =0}^{N-1} \omega_N^{kj} z_k = \frac{1}{\sqrt{N}} \left(z_0 + z_{N/2} + \sum_{k = 1}^{N/2-1} \omega_N^{kj} z_k + \sum_{l = N/2+1}^{N-1} \omega_N^{lj} z_l\right)
    ...
    = \frac{1}{\sqrt{N}} \left(z_0 + z_{N/2} + \sum_{k = 1}^{N/2-1} \left(\omega_N^{kj} z_k + \overline{\omega_N^{kj}z_k}\right)\right) \in \R\]
