...
\begin{proof}
	...
	If $\deg P = N > M = \deg Q$, then the Lemma~\ref{lm:long div} ensures that exist $q(z^-1), r(z^-1)$ with $\deg q \leq N-M$ and $\deg r \leq M-1$, s.t. $P(z^{-1}) = q(z^{-1})Q(z^{-1}) + r(z^{-1})$.
	\[X(z) 
	...
	\sum_{n=-\infty}^{\infty} \sum_{i = 0}^{N-M} |c_i\delta[n-i]| + \sum_{i=1}^{M} |A_i d_i^n u[n] |
	\\ &= \sum_{i = 0}^{N-M} |c_i| + \sum_{n=-\infty}^{\infty} \sum_{i=1}^{M} |A_i d_i^n u[n]|
	\end{align*}

	We require each sum $\sum_{n=-\infty}^{\infty} |A_i d_i^n u[n]|$ for $\{1, \dots, M\}$ to converge, as this would imply that $\sum_{n=-\infty}^{\infty} \sum_{i=1}^{M} |A_i d_i^n u[n]|$ converges as well by limit additivity.
	\[\sum_{n=-\infty}^{\infty} |A_i d_i^n u[n]| 
	= |A_i| \sum_{n=0}^{\infty} |d_i^n| < \infty\]
	This is the sum of geometric series, hence converges for $d_i$ with $|d_i|<1$. We need this to hold for all $i\in \{1, \dots, M\}$, so $\max_{i} |d_i| < 1$
\end{proof}

We say that z-transform converges if the series
\[\sum_{n=-\infty}^{\infty} |x[n]z^{-n}| = \sum_{n=-\infty}^{\infty} |x[n]|r^{-n}\]
converges (\todo{reference openheim 3.6 for completeness}). It can be easily seen that if the z-transform converges for some point located at the distance $r$ to the origin, then it converges for any point at the circle with radius $r$ centered at the origin. This immediately yields the following equivalence:
